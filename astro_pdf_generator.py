...
Lucky Stone: {elem['stone']}"""
    ...
    # Calculate Loshu Grid from birth date
    date_digits = [int(d) for d in birth_date.replace('-', '')]
    ...
    for row in loshu_grid:
        ...
        for num in row:
            if num in digit_count:
                # Show repeated digits (e.g., "22" for 2 occurrences)
                display = str(num) * digit_count[num]
            else:
                display = "-"
            row_values.append(display.center(6))
        grid_text += "  ".join(row_values) + "\n\n"
    
    pdf.set_font('Courier', '', 12)  # Monospace font for grid
    pdf.multi_cell(0, 8, grid_text)
    pdf.set_font('Arial', '', 11)  # Back to normal font
    ...
    if ln:
        analysis = "Grid Vishleshan:\n"
    else:
        analysis = "Grid Analysis:\n"
    
    if missing_numbers:
        if ln:
            analysis += f"Kami Sankhya: {', '.join(map(str, missing_numbers))}\n"
        else:
            analysis += f"Missing Numbers: {', '.join(map(str, missing_numbers))}\n"
    else:
        if ln:
            analysis += "Kami Sankhya: Koi nahi\n"
        else:
            analysis += "Missing Numbers: None\n"
    
    if repeated_numbers:
        rep_str = ', '.join([f"{k} ({v} times)" for k, v in repeated_numbers.items()])
        if ln:
            analysis += f"Dohrai Sankhya: {rep_str}\n"
        else:
            analysis += f"Repeated Numbers: {rep_str}\n"
    else:
        if ln:
            analysis += "Dohrai Sankhya: Koi nahi\n"
        else:
            analysis += "Repeated Numbers: None\n"
    ...
    # ====================
    if ln:
        pdf.section_title('Bhagyashali Aur Shatru Sankhya')
    else:
        pdf.section_title('Lucky & Enemy Numbers')
    
    # Lucky numbers based on personality number
    lucky_numbers_map = {
        1: [1, 2, 3, 9],
        2: [1, 2, 7],
        3: [1, 3, 5, 6, 9],
    ...
    lucky_nums = lucky_numbers_map.get(personality_number, [])
    enemy_nums = enemy_numbers_map.get(personality_number, [])
    
    if ln:
        numbers_text = f"""Bhagyashali Sankhya: {', '.join(map(str, lucky_nums)) if lucky_nums else 'Koi nahi'}

Shatru Sankhya: {', '.join(map(str, enemy_nums)) if enemy_nums else 'Koi nahi'}"""
    else:
        numbers_text = f"""Lucky Numbers: {', '.join(map(str, lucky_nums)) if lucky_nums else 'None'}

Enemy Numbers: {', '.join(map(str, enemy_nums)) if enemy_nums else 'None'}"""
